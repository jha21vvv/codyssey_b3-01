...
* **무엇을 보여주기 위한 것인가? (증빙 목적)**
  * 가상 머신이 사전에 구성한 퍼블릭 서브넷(`codyssey-public-subnet-2a`)에 올바르게 잘ㅜ 생성되었고, AWS 시스템 및 인스턴스 하드웨어 검증을 모두 통과(`3/3 검사 통과`)하여 정상 가동 중이며, 외부에서 접근 가능한 고유 퍼블릭 IPv4를 보유하고 있음을 증명하기 위함입니다.
* **무엇을 찍어 어떻게 증명하였는가? (증빙 내용 및 핵심 확인 요소)**
  * **확인 화면**: AWS 콘솔 > EC2 > [인스턴스] > `codyssey-web-server` 상세 카드
  * **핵심 확인 포인트**:
    1. **인스턴스 상태**: 초록색 `실행 중 (running)` 및 상태 검사 `3/3 검사 통과` 표시 확인.
...
## 🎓 5. 클라우드 인프라 핵심 및 심화 질의응답 (Technical Q&A)

---

### Q1. 실습 리소스를 추적/정리하기 위해 어떤 기준(태그, 이름 규칙, 체크리스트)을 사용했는지 설명할 수 있는가?
* **1. 명확한 접두사 네이밍 규칙 (Naming Convention)**:
  * 본 실습에서 생성한 모든 클라우드 자원에 프로젝트 공통 식별자인 `codyssey-` 접두사를 일관되게 적용하였습니다.
  * 예: `codyssey-vpc`, `codyssey-public-subnet-2a`, `codyssey-igw`, `codyssey-public-rt`, `codyssey-web-sg2`, `codyssey-web-server`.
  * 이를 통해 계정에 기본 존재하는 디폴트 VPC/서브넷과 실습용 자원을 콘솔 검색창에서 즉시 구분할 수 있었습니다.

* **3. 리소스 역종속성 정리 체크리스트 (`docs/cleanup-checklist.md`) 운용**:
  * 생성 순서의 정반대인 **[EC2 가상머신 종료 ➔ EBS 루트 볼륨 소멸 ➔ 탄력적 IP 미할당 ➔ IGW 분리 및 삭제 ➔ 서브넷/라우팅 삭제 ➔ 보안 그룹 삭제 ➔ VPC 삭제]**의 7단계 체크리스트를 수립하고, 식별 ID와 상태를 1:1 대조하며 안전하게 회수하였습니다.

---

### Q2. Public Subnet Route Table에 기본 경로(`0.0.0.0/0 → IGW`)가 필요한 이유를 설명할 수 있는가?
* **1. VPC 라우팅의 기본 한계**:
  * 라우팅 테이블 생성 시 기본 탑재된 규칙은 오직 사설망 내부 통신용인 `10.0.0.0/16 ➔ local`뿐입니다. 따라서 서브넷은 VPC 외부 세상으로 나가는 길을 알지 못합니다.
* **2. `0.0.0.0/0` (기본 경로, Default Route)의 의미**:
  * 네트워크에서 `0.0.0.0/0`은 **"로컬 사설망 대역에 속하지 않는 전 세계 모든 외부 IP 목적지"**를 뜻합니다.
* **3. 왜 IGW를 타깃으로 지정해야 하는가?**:
  * EC2 서버가 패키지 업데이트(`apt update`)를 위해 인터넷으로 패킷을 보내거나, 외부 사용자 브라우저의 HTTP 요청에 200 OK 답장 패킷을 보내려면 외부 관문인 **Internet Gateway(IGW)**로 나가야 합니다.
  * 따라서 `0.0.0.0/0 ➔ codyssey-igw` 규칙이 명시되어 있어야만 양방향 외부 통신이 활성화되며, 이 규칙이 연결된 서브넷만이 비로소 **'퍼블릭 서브넷(Public Subnet)'**으로 기능할 수 있습니다.

---

### Q3. Security Group과 IAM의 책임 범위 차이를 설명하고, "최소 권한"이 왜 필요한지 설명할 수 있는가?
* **1. 책임 범위의 본질적 차이**:
  * **Security Group (네트워크 레벨 방화벽)**: 가상 랜카드(ENI) 단위에서 패킷을 검사합니다. "어떤 IP가 몇 번 포트(예: 80, 22)로 접근할 수 있는가?"를 통제하는 Stateful 가상 방화벽입니다.
  * **IAM (리소스/API 레벨 신원·권한 관리)**: 작업자나 프로그램(User/Role) 단위에서 조작 권한을 검사합니다. "누가 AWS 콘솔이나 API를 통해 EC2를 생성/삭제하고, VPC를 변경할 수 있는가?"를 통제합니다.
* **2. "최소 권한의 원칙(Least Privilege)"이 필요한 이유**:
  * 계정 자격 증명이 해킹당하거나 관리자의 단순 조작 실수(Human Error)가 발생했을 때 **시스템 전체로 피해가 확산되는 것을 차단하고 피해 반경(Blast Radius, 폭발 반경)을 최소화**하기 위함입니다.
  * **보안 그룹 관점**: 관리자 포트(SSH 22)를 전체 개방(`0.0.0.0/0`)하면 무차별 대입 공격에 노출되므로 관리자 IP(`/32`)만 허용.
  * **IAM 관점**: 모든 권한(`AdministratorAccess`) 대신 실습에 필수적인 EC2/VPC 제어 권한만 부여하여 무단 결제 및 타 서비스 침해 원천 차단.

---

### Q4. 외부 접속이 안 될 때, 어떤 순서로(라우팅 → SG → 퍼블릭 IP/DNS → 서버 프로세스/로그) 점검하는지 설명할 수 있는가?
계층별 네트워크 디버깅 원칙에 따라 **"안쪽 서버 프로세스부터 바깥쪽 네트워크 관문까지"** 체계적으로 원인을 좁혀가며 점검합니다:

1. **[1단계] 서버 프로세스 및 로컬 응답 검증 (OS & Process Layer)**:
   * EC2 터미널에서 `sudo systemctl status nginx`를 실행하여 Nginx 웹 데몬이 `Active (running)` 상태인지 확인.
   * `curl -I http://localhost` 루프백 테스트를 통해 서버 내부에서 HTTP 200 OK가 정상 반환되는지 확인 (실패 시 Nginx 에러 로그 `/var/log/nginx/error.log` 확인).
2. **[2단계] 보안 그룹 방화벽 인바운드 규칙 점검 (Security Group Layer)**:
   * AWS 콘솔에서 인스턴스에 적용된 보안 그룹의 인바운드 규칙 확인.
   * 웹 서비스용 **HTTP(TCP 80번) 포트가 `0.0.0.0/0`에 열려 있는지 확인** (실수로 HTTPS 443번이나 엉뚱한 포트가 등록되었는지 검증).
3. **[3단계] 공인 IP 및 가상 랜카드(ENI) 점검 (Public IP & Addressing Layer)**:
   * 인스턴스에 퍼블릭 IPv4 주소(`3.36.130.0`)가 올바르게 바인딩되어 있는지, 외부에서 도달 가능한 공인 IP인지 확인.
4. **[4단계] VPC 네트워크 및 라우팅 경로 점검 (Routing & Gateway Layer)**:
   * 인스턴스가 위치한 서브넷의 라우팅 테이블을 열어 `0.0.0.0/0`의 대상이 인터넷 게이트웨이(`igw-xxxx`)로 지정되어 있는지 확인.
   * 인터넷 게이트웨이가 VPC에 정상 `Attached` 상태인지 최종 점검.

---

### Q5. IAM 권한이 부족해 작업이 실패했다면, "권한을 무작정 늘리지 않고" 어떤 방식으로 필요한 최소 항목을 찾아낼지 설명할 수 있는가?
* **1. 콘솔 및 터미널의 에러 로그 메시지 정밀 분석 (Error Signature Parsing)**:
  * AWS API 거부 에러는 항상 구체적인 누락 API를 알려줍니다.
  * 예: `User is not authorized to perform: ec2:DescribeDhcpOptions on resource: *` ➡️ 이를 통해 실패한 정확한 액션(`ec2:DescribeDhcpOptions`)을 핀포인트로 식별.
* **2. 작업 성격에 따른 권한 분리 적용 (Read vs Write Separation)**:
  * 단순 화면 렌더링을 위한 읽기 오류(`Describe*`)라면, 과금이나 리소스 파괴 위험이 없으므로 `ec2:Describe*` 구문에 안전하게 포함.
  * 상태를 변경하거나 리소스를 수정하는 명령(`Modify*`, `Create*`, `Delete*`)이라면, 무작정 `ec2:*`를 주지 않고 필요한 액션만 단건 단위로 정책(JSON)에 추가.
* **3. AWS CloudTrail 이벤트 기록(Event History) 역추적**:
  * 콘솔에 에러 코드가 자세히 안 나올 경우, CloudTrail에서 `이벤트 이름`과 `errorCode: Client.UnauthorizedOperation / AccessDenied`를 검색하여 거부된 API 호출을 정확히 확인.

---

### Q6. 트래픽이 늘어 "인스턴스를 2대로 늘려야" 한다면, 현재 구조에서 무엇이 병목이며 어떤 구성요소(ALB 등)를 추가할지 설명할 수 있는가?
* **1. 현재 아키텍처의 한계 및 병목 (Single Point of Failure)**:
  * **단일 장애점 (SPOF)**: EC2 인스턴스가 단 1대뿐이므로, 하드웨어 장애나 프로세스 크래시 발생 시 서비스 전체가 완전 중단됨.
  * **직통 IP 접속 한계**: 외부 사용자가 EC2 인스턴스의 고유 공인 IP(`3.36.130.0`)로 직접 들어오고 있으므로, 서버를 2대로 늘려도 사용자를 분산시킬 방법이 없음.
* **2. 확장 시 추가해야 할 핵심 구성요소**:
  1. **Application Load Balancer (ALB, 애플리케이션 부하 분산기)**:
     * 퍼블릭 서브넷에 ALB를 배치하고, 외부 사용자는 ALB의 도메인(DNS)으로 접속하게 함.
     * ALB가 뒤쪽의 EC2 인스턴스 2대에 웹 트래픽을 균등하게 분산(Round Robin)하고, 주기적 헬스체크를 수행하여 장애 난 인스턴스로는 트래픽을 보내지 않음.
     - 일종의 번호표로 부하가 양쪽에 뀬등하게 배분되도록 부하를 줄이는것
  2. **다중 가용 영역(Multi-AZ) 서브넷 확장**:
     * 현재는 1개 가용영역(`ap-northeast-2a`)에만 서브넷이 존재함. 데이터센터 정전에 대비하여 `ap-northeast-2c` 등에 서브넷을 추가하고 인스턴스를 각각 분산 배치하여 고가용성(HA) 확보.
  3. **보안성 고도화 (프라이빗 서브넷 격리)**:
     * 웹 서버 EC2 인스턴스들을 외부 인터넷에 직접 노출하지 않고 **프라이빗 서브넷(Private Subnet)**으로 이동시킨 뒤, 오직 ALB를 통해서만 트래픽이 들어오도록 보안 그룹을 강화.

---

### Q7. Billing에 예상치 못한 비용이 찍혔다면, 어떤 리소스부터 의심하고 어떻게 추적/정리할지 설명할 수 있는가?
* **1. 1순위 의심 대상 리소스 (클라우드 유휴 과금 4대 주범)**:
  1. **미연결 유휴 탄력적 IP (Unattached Elastic IP)**: 인스턴스에 연결되지 않은 채 발급만 받아둔 EIP는 시간당 패널티 요금 발생.
  2. **종료된 EC2의 고아 EBS 볼륨 (Orphaned EBS Volumes)**: EC2 인스턴스만 끄고 연결되어 있던 하드디스크 볼륨을 지우지 않으면 기가바이트당 월간 보관료 지속 청구.
  3. **미삭제 NAT Gateway**: 생성 후 방치 시 시간당 약 $0.059(한 달 4~5만 원 상당)의 높은 고정 요금 발생.
  4. **고스펙 관리형 서비스 잔존**: 실수로 생성한 RDS DB 인스턴스, ALB 로드밸런서 등.
* **2. 비용 추적 및 식별 방법**:
  * **AWS Billing > 청구서(Bills)**: 리전별 세부 청구서 라인 항목(예: `EBS - gp3`, `Elastic IP`)을 열어 어떤 서비스에서 몇 달러가 청구되었는지 즉시 확인.
  * **AWS Cost Explorer (비용 탐색기)**: 서비스(Service)별 및 사용 유형(Usage Type)별 차트를 통해 특정 일자에 비용이 급증한 리소스를 시각적으로 추적.
* **3. 정리 및 조치**:
  * 추적된 리소스 콘솔로 이동하여 유휴 EIP는 `Release(릴리스)`, 미연결 볼륨은 `Delete Volume`, 잔존 NAT Gateway나 인스턴스는 즉시 `Terminate/Delete`하여 과금 요소를 완전히 제거함.